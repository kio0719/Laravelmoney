...
@if(!($items->isEmpty()))
<table>
    <tr><th></th><th>使用日</th><th>引落日</th><th>収支区分</th><th>勘定科目</th><th>金額</th><th>備考</th></tr>
    @foreach($items as $item)
    <tr>
 <!--   <td><input type="radio" name="asset_select" value="{{$item['asset_id']}}"></td> 
-->
<td><a href="{{route('log.getchange',['atlog_select' => $item['log_id'] ])}}"><input type="button" value="変更する"></a> | 
<a href="{{route('log.getdelete',['atlog_select' => $item['log_id'] ])}}"><input type="button" value="削除する"></a></td>     
        <td>{{$item['use_date']}}</td>
